...
const RegisterScreen = (route) => {
    ...
    const onChangeUser = (dato) => {
        ...
        for (var i = 0; i < dato.length; i++) {
            if (caracter.indexOf(dato[i]) > -1 || (dato[i] >= 0 && dato[i] <= 9)) {
                //Aceptamos solo si el caracter se encuentra
                if (i < 25) {
                    //Solo permitimos hasta 25 caracteres
                    nwdato += dato[i];
                }
            }
        }
    ...
    const onChangePassword = (dato) => {
        ...
        for (var i = 0; i < dato.length; i++) {
            if (i < 50) {
                //Solo permitimos hasta 50 caracteres
                nwdato += dato[i];
            }
        }
        setPassword(nwdato);
                    ...
                    <View style={[Styles.viewbtndecor, Styles.grpbtn]}>
                        ...
                        <TouchableOpacity onPress={() => auth()}>
                            <Text style={Styles.olvidar}>
                                <FontAwesome5 name="angle-double-left" /> Posees una cuenta?
                                Iniciar Sesión <FontAwesome5 name="angle-double-right" />
                            </Text>
                        </TouchableOpacity>
                    </View>
                </View>
