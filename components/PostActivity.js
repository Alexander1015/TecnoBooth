...
const Post = ({ post, verificado, idUsuario }) => {
    const { descripcion, img, usuario, id } = post;

    const { obtenerComentarios, comentarios } = useGrupos();
    ...
    return (
        <View style={styles.container}>
            ...
            <TouchableOpacity
                ...
                onPress={() => setVerComentario(!verComentario)}
            >
                <Text style={[styles.texto, { color: "white" }]}>
                    {verComentario ? "Ocultar Comentarios" : "Ver Comentarios"}
                </Text>
            </TouchableOpacity>
